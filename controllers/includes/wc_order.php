...
class JoApp_WC_API_Orders {
    ...
    public function get_order($id, $fields = null, $filter = array()) {
        ...
        $order_data = array(
            ...
            'payment_details' => array(
                ...
                'paid' => !is_null($order->get_date_paid())//,isset($order->paid_date),
            ),
    ...
    protected function set_item($order, $item_type, $item, $action) {
        ...
        if ('update' === $action) {
            ...
            $result = $wpdb->get_row(
                    $wpdb->prepare("SELECT * FROM {$wpdb->prefix}woocommerce_order_items WHERE order_item_id = %d AND order_id = %d", absint($item['id']), absint($order->id)
            ));
     ...
     * @throws WC_API_Exception invalid data, server error
     */
    public function set_shipping($order, $shipping, $action) {
